...
namespace UnityEngine.Experimental.Rendering
{
    ...
    public class ShadowAtlas : ShadowmapBase, IDisposable
    {
        ...
        override public bool Reserve(FrameId frameId, Camera camera, bool cameraRelativeRendering, ref ShadowData shadowData, ShadowRequest sr, uint width, uint height, ref VectorArray<ShadowData> entries, ref VectorArray<ShadowPayload> payload, List<VisibleLight> lights, ShadowmapRenderFlags shadowmapRenderFlags)
        {
        ...
        override public bool Reserve(FrameId frameId, Camera camera, bool cameraRelativeRendering, ref ShadowData shadowData, ShadowRequest sr, uint[] widths, uint[] heights, ref VectorArray<ShadowData> entries, ref VectorArray<ShadowPayload> payload, List<VisibleLight> lights, ShadowmapRenderFlags shadowmapRenderFlags)
        {
           ...
           // uint originalActiveEntries = m_ActiveEntriesCount;
            ...
            while (facecnt > 0)
            {
                if ((bit & facemask) != 0)
                {
                    ...
                    {
						if((ce.current.shadowmapRenderFlags & ShadowmapRenderFlags.DontRefresh) == 0 || ce.previousRect.width == 0.0f)
						{
                    ...
                    sd.normalBias = new Vector4((asd.shadowmapRenderFlags & ShadowmapRenderFlags.DontRefresh) != 0 && (asd.shadowmapRenderFlags & ShadowmapRenderFlags.RenderDynamicRenderers) != 0 ? Mathf.Max(asd.normalBiasMin, 6.0f) : asd.normalBiasMin, asd.normalBiasMax, asd.normalBiasScale, ShadowUtils.Asfloat(flags));
        ...
        override public void Update(FrameId frameId, ScriptableRenderContext renderContext, CommandBuffer cmd, CullResults cullResults, List<VisibleLight> lights)
        {
            ...
            for (uint i = 0; i < m_ActiveEntriesCount; ++i)
            {
				...
				if ((entryCache.current.shadowmapRenderFlags & ShadowmapRenderFlags.CacheStaticRenderers) != 0)
	            {
		            ...
		            if (cachedTex && !cachedTex.IsCreated())
		            {
			            asd.cachedShadowmapFaces = 0;
			            asd.cachedShadowmapFacesHiRes = 0;
			            cachedTex.Create();

			            if (asd.cachedShadowmap2 && !asd.cachedShadowmap2.IsCreated())
				            asd.cachedShadowmap2.Create();

			            if (asd.cachedShadowmapHiRes != null)
			            {
				            for(int ii=0; ii<asd.cachedShadowmapHiRes.Length; ++ii)
				            {
					            if(asd.cachedShadowmapHiRes[ii] && !asd.cachedShadowmapHiRes[ii].IsCreated())
						            asd.cachedShadowmapHiRes[ii].Create();
							}
						}
		            }
		            ...
		            int res = (int)entryCache.current.viewport.width;

					if (((res <= 256 ? asd.cachedShadowmapFaces : asd.cachedShadowmapFacesHiRes) & (1 << faceIdx)) == 0)
					{
						if (res <= 256)
							asd.cachedShadowmapFaces |= 1 << faceIdx;
						else
							asd.cachedShadowmapFacesHiRes |= 1 << faceIdx;

						if (!cachedTex && res <= 256)
						{
							switch (light.type)
				            {
					            case LightType.Point:
						            if (res >= 256)
						            {
							            asd.cachedShadowmap = cachedTex = CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap");
							            asd.cachedShadowmap2 = CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap");
							            asd.cachedShadowmapWidth = 512;
							            asd.cachedShadowmapHeight = 512;
						            }
						            else
						            {
			            ...
			            if (res >= 512)
			            {
				            if (asd.cachedShadowmapHiRes == null)
				            {
								asd.cachedShadowmapHiRes = new[]
								{
									CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap 0"),
									CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap 1"),
									CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap 2"),
									CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap 3"),
									CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap 4"),
									CreateCachedShadowmap(512, 512, light.name + " Cached Shadowmap 5")
								};
							...
							cachedTex = asd.cachedShadowmapHiRes[faceIdx];
				            rect = new Rect(0.0f, 0.0f, 1.0f, 1.0f);
						}
			            else if (asd.cachedShadowmapWidth == 512 && asd.cachedShadowmapHeight == 512)
			            {
				            cachedTex = faceIdx < 3 ? asd.cachedShadowmap : asd.cachedShadowmap2;
							rect = faceViewports512[faceIdx];
			            }
			            ...
			            if (CullResults.GetCullingParameters(tempCamera, out cullingParams))
			            {
				            cullingParams.cullingMask = ~light.cullingMask;
				            cullingParams.shadowDistance = 0.0f;
				            cullingParams.reflectionProbeSortOptions = ReflectionProbeSortOptions.None;
				            cullingParams.cullingFlags = CullFlag.DisablePerObjectCulling | CullFlag.ForceEvenIfCameraIsNotActive;

				            var cullResults2 = new CullResults();

				            CullResults.Cull(ref cullingParams, renderContext, ref cullResults2);
				            // cull stop
				            ...
				            drawSettings.SetShaderPassName(0, new ShaderPassName("ShadowCaster"));

				            var filterSettings = new FilterRenderersSettings
				            {
					            layerMask = ~light.cullingMask,
					            renderingLayerMask = uint.MaxValue,
					            renderQueueRange = new RenderQueueRange {min = 0, max = 2800}
				            };
				            renderContext.DrawRenderers(cullResults2.visibleRenderers, ref drawSettings, filterSettings);
			            }
					...
					if(res >= 512 && asd.cachedShadowmapHiRes != null)
					{
						cachedTex = asd.cachedShadowmapHiRes[faceIdx];
						rect = new Rect(0.0f, 0.0f, 1.0f, 1.0f);
					}
	    ...
	    private RenderTexture CreateCachedShadowmap(int width, int height, string name)
	    {
...
// -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    public class ShadowManager : ShadowManagerBase
    {
        ...
        protected override void PrioritizeShadowCasters(Camera camera, List<VisibleLight> lights, uint shadowRequestsCount, int[] shadowRequests)
        {
            ...
            m_TmpSortKeys.Sort(RerverseCompare);
