...
#pragma warning restore 414
		...
		[NonSerialized] public RenderTexture cachedShadowmap2;
		[NonSerialized] public RenderTexture[] cachedShadowmapHiRes;
		...
		[NonSerialized] public int cachedShadowmapFacesHiRes;
	    ...
	    public void ClearShadowmapCache()
	    {
		    ...
		    {
			    if(Application.isPlaying)
				    Destroy(cachedShadowmap2);
			    else
				    DestroyImmediate(cachedShadowmap2);
		    }

		    if(cachedShadowmapHiRes != null)
		    {
			    for(int i = 0; i < cachedShadowmapHiRes.Length; ++i)
				    Destroy(cachedShadowmapHiRes[i]);

			    cachedShadowmapHiRes = null;
		    }
	    }
